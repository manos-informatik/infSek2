(() => {
  ...
  const parseDatei = (datei, programm) => {
    ...
    /* Modifikatoren und Annotationen wie @Override */
    const modifikatoren = () => {
      const gefunden = new Set();
      while ((sieh().typ === "wort" && MODIFIKATOREN.has(sieh().text)) || sieh().typ === "annotation") {
        gefunden.add(weiter().text);
      }
      return gefunden;
    };
    ...
    const klasse = (mods) => {
      ...
      while (!ist("}")) {
        ...
        if (ist("(")) {
          ...
          const ueberschreibt = m.has("@Override");
          fuegeHinzu(k.methoden, { name: mn.text, typ: typText, params, rumpf, token: mn, klasse: k, abstrakt, ueberschreibt }, "Methode");
        } else {
  ...
  const verknuepfe = (programm) => {
    ...
    /* @Override: die Methode muss es in einer Oberklasse schon geben */
    programm.klassen.forEach((k) => {
      k.methoden.forEach((liste) => liste.forEach((m) => {
        if (!m.ueberschreibt || OBJEKT_METHODEN.has(m.name)) return;
        if (!findeMethode(k.oberklasse, m.name, m.params.length, true)) {
          throw new SketchFehler(`„${m.name}()“ hat @Override, überschreibt aber keine Methode einer Oberklasse.`, m.token);
        }
      }));
    });
